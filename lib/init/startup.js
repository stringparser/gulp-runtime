...
// wait time to prompt on tasks stop
runtime.config('wait', util.type(Math.abs(Number(argv.wait))).integer || 50);
console.log(util.type(Math.abs(Number(argv.wait))).integer || 50);
console.log(runtime.config('wait'));
var status = { done : [] };
runtime.on('st', function statusDone(e){
  ...
  if( e.duration ){
    ...
    status = { done : [ ] };
    setTimeout(function(){
      if( argv.repl !== 'false' ){  runtime.prompt(); }
    }, runtime.config('wait') );
  }
});
