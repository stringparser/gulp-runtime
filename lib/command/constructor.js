...
function Command(cmd){
  ...
  this.version = function(number, comment){
    ...
    args[0] ?
      util.merge(version, {  number : args[0].join(' ') }) : null;

    args[1] ?
      util.merge(version, { comment : args[1].join(' ') }) : null;
...
function isOptions(args, method, anchor){
  ...
  if( util.isUndefined(arg) )     return arg;

  if(typeof arg === 'function') arg = arg();
  if(typeof arg === 'string')   arg = arg.trim().split(/[ ]+/);
  ...
  if(!Array.isArray(arg)){
    ...
    throw RuntimeError(
     + ' Argument `'+arg+'` not supported for "' + anchor._name+'"' + '\n'
