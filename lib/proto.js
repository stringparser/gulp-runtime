...
Stack.prototype.onHandleNotFound = function(next){
  ...
  util.log('%s found for `%s`', util.color.red('No task'), path);
  util.log('Set one with `task(\'%s\', [Function])`', path);
  this.log = false;
  process.exit(1);
};
...
Runtime.prototype.task = function(name, deps, handle){
  ...
  // since we have path-to-regex mapping
  // TODO: do not allow path with params to be deps?
  //
