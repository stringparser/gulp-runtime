...
Runtime.prototype.task = function(name, deps, handle){
  ...
  // TODO: this might to much overhead, think of a better solution
  //
  deps.forEach(function isDepCircular(dep){
    var node = this.get(dep, {ref: true});
    if(node.path !== name){ return ; }

    dep = util.color.red(dep);
    name = util.color.cyan(name);
    regex = util.color.yellow(regex);

    var regex = RegExp(this.regex.filter(function(re){
      return re.path === name;
    })[0].source);

    var error = new Error(
      'task `'+name+'` dependency `'+dep+'` is circular\n'+
      'Meaning: if is run it will call itself\n'+
      'Solution: rewrite it so they don\'t match\n'+
      '- path: ' + name + '\n'+
      '- deps: ' + JSON.stringify(deps) + '\n'+
      '- regex: ' + regex +'\n--'
    );
    // mark caller's file path on the stack
    util.color.callersPath(error, 2);
    throw error;
  }, this);
...
Runtime.prototype.watch = function(glob, opt, cb){
  ...
  opt = !cb && util.type(cb).plainObject
    || cb !== opt && util.type(opt).plainObject
    || {tasks: opt};

  if(cb || opt.reload){
    opt.onHandleEnd = function(ev){
      if(util.reloadFile(opt, ev)){
  ...
  var tasks = util.type(opt && opt.tasks || opt);
  if(tasks.string || tasks.array){
    tasks = tasks.string || tasks.array.join(' ');
    return vinylFS.watch(glob, null, this.stack(tasks, opt));
  }
