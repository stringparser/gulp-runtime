...
Runtime.prototype.task = function(name, dep, handle){
  ...
  if(!handle.length && !/return/.test(handle.toString())){
    ...
    var error = new Error(
      ...
      'Meaning: the task function does not return nor uses a callback\n'+
      'Solution:'+
      ' - return a stream, promise or observable\n'+
      ' - use the callback that is passed as first argument\n--'
    );
    var file = error.stack.match(/([^()]+:\d+:\d+)/g)[1];
    error.stack = error.stack.replace(file, util.color.file(file)) + '\n';
    throw error;
  }
