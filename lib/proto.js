...
Runtime.prototype.task = function(name, dep, handle){
  ...
  // TODO
  //  - find a good way of checking handle completion
  //
  if(!handle.length && !/return/.test(handle.toString())){
    var stack = new Error().stack;
    console.log(
      '\nFrom: %s\n'+
      'Message: task `%s` doesn\'t give a hint of its completion.\n'+
      'Meaning: task function does not return nor uses a callback.\n--\n'+
      'To make it work you can either:\n'+
      ' - return a stream, promise or observable\n'+
      ' - use the callback that is passed as first argument\n',
      util.color.file(stack.split('\n')[2].match(/\((.+)\)/)[1]),
      util.color.cyan(name)
    );
    process.exit(1);
  }
